...
{
	int (*recv)(Network*, unsigned char* read_buffer, int, int);
	int (*send)(Network*, unsigned char* send_buffer, int, int);
} Network;*/

/* The MqttTimer structure must be defined in the platform specific header,
 * and have the following functions to operate on it.  */
extern void MqttTimerInit(MqttTimer*);
extern char MqttTimerIsExpired(MqttTimer*);
extern void MqttTimerCountdownMS(MqttTimer*, unsigned int);
extern void MqttTimerCountdown(MqttTimer*, unsigned int);
extern int MqttTimerLeftMS(MqttTimer*);
 extern int  init_mqtt_module();

#define MutexLock(x)  do { (x)->lock(); }while(0)
#define MutexUnlock(x)  do { (x)->unlock(); }while(0)
...
extern volatile MQTTClient* getSysMqttClient();
#endif
